...
#Load the dataset
caps <- as.data.frame(read.csv("SEA_Activity_SEA_captures_extreme_20230523.csv"))
...
#Save it!
write.csv(df, "SEA_Activity_raw_total_detections_extreme_20230531.csv", row.names = F)

perc <- df %>% 
  group_by(species_composition) %>% 
  summarise(perc_change_day = (day[FLII_status == "degraded"]/(total_detections[FLII_status == "degraded"] + total_detections[FLII_status == "intact"]) -
                                 day[FLII_status == "intact"]/(total_detections[FLII_status == "degraded"] + total_detections[FLII_status == "intact"]))*100, 
            perc_change_crep = (crepuscular[FLII_status == "degraded"]/(total_detections[FLII_status == "degraded"] + total_detections[FLII_status == "intact"]) -
                                  crepuscular[FLII_status == "intact"]/(total_detections[FLII_status == "degraded"] + total_detections[FLII_status == "intact"]))*100, 
            perc_change_noc = (night[FLII_status == "degraded"]/(total_detections[FLII_status == "degraded"] + total_detections[FLII_status == "intact"]) -
                                 night[FLII_status == "intact"]/(total_detections[FLII_status == "degraded"] + total_detections[FLII_status == "intact"]))*100)


perc$species_composition <- c("all_species", "no_pigs_macaques", "only_pigs_macaques")

write.csv(perc, "changes_in_perc_detections_extreme_20230531.csv", row.names = F)
...
guilds <- guilds[-1] #remove large carnivores 
...
#save it
write.csv(df, "SEA_Activity_raw_guild_detections_extreme_20230605.csv", row.names = F)
...
perc_guild <- df %>% 
  ...
  summarise(perc_change_day = (day[FLII_status == "degraded"]/total_detections[FLII_status == "degraded"]) - 
            ...
            perc_change_night = (night[FLII_status == "degraded"]/total_detections[FLII_status == "degraded"]) - 
              (night[FLII_status == "intact"]/total_detections[FLII_status == "intact"]))

perc_guild$guild <- guilds

#save this!
write.csv(perc_guild, "SEA_Activity_guild_perc_change_extreme_20230605.csv", row.names = F)
