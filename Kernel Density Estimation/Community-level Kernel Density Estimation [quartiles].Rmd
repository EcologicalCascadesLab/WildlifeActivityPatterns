---
title: "Community-level Temporal Shift Analyses (Extremes)"
...
caps = as.data.frame(read.csv("C:/Users/AMD/Dropbox/Sam Lee Honours/SEA Activity Data Analysis Sam Honours/SEA Activity Temporal Shift Analysis Sam Honours/SEA Activity temporal shift by community Sam Honours/SEA_Activity_dataset_for_community_guild-level_analyses_extreme_20230523.csv"))
...
flii$status = NA
flii$status[flii$forest_integrity < 5.61] = "degraded"
flii$status[flii$forest_integrity > 5.61] = "intact"
...
#Save the plot!
ggsave("FLII_extreme_distribution.pdf", plot = p, width = 8, height = 6, units = "in")
...
#Keep environment clean!
rm(all_species_list,no_pigs_macaques_list,only_pigs_macaques_list)
```
...
saveRDS(actmod.list, 'SEA_Activity_community-level_actmods_extreme_20230523.RDS')
...
write.csv(df, 'SEA_Activity_community-level_results_extreme_20230529.csv', row.names = F)
...
for(i in 1:length(cov_plot)){ #repeat for each treatment
  ...
  path = paste0(b, "_extreme_20230529.PDF", sep = "")
...
#Keep environment clean
rm(a,b,path,i, cov_plot, pdf_cov)
```
